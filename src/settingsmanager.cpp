...
static const SettingDefinition settingDefinitions[] = {
    ...
    // AI Rename settings
    { SettingsManager::Setting::AIRenameAPIKey, "", "airename/apikey" },
    { SettingsManager::Setting::AIRenameBaseURL, "https://api.openai.com/v1", "airename/baseurl" },
    { SettingsManager::Setting::AIRenameModel, "gpt-4-vision-preview", "airename/model" }
};
...
{
    qDebug() << "Getting setting - Key:" << key << "Defaults:" << defaults;
    
    // Find the setting enum that matches this key
    for (const auto &def : settingDefinitions) {
        if (QString(def.key) == key) {
            qDebug() << "Found setting definition - Key:" << def.key << "DefaultValue:" << def.defaultValue;
