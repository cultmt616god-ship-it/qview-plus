...
{
    qDebug() << "=== Modifying Setting === Key:" << key << "Value:" << value;
...
void QVOptionsDialog::saveSettings()
{
    ...
    for (const auto &key : keys) {
        ...
        qDebug() << "Saving Setting - Key:" << key << "Value:" << value;
...
void QVOptionsDialog::syncSettings(bool defaults, bool makeConnections)
{
    ...
    // AI Rename settings
    syncLineEdit(ui->apiKeyEdit, "airename/apikey", defaults, makeConnections);
    syncLineEdit(ui->baseUrlEdit, "airename/baseurl", defaults, makeConnections);
    syncLineEdit(ui->modelEdit, "airename/model", defaults, makeConnections);
}
                                 ...
                                 bool makeConnection)
{
    ...
    // Debug information
    qDebug() << "SyncLineEdit - Key:" << key << "Value:" << val << "Defaults:" << defaults;

    if (makeConnection) {
        connect(lineEdit, &QLineEdit::textChanged, this,
                [this, key](const QString &arg1) { 
                    qDebug() << "LineEdit changed - Key:" << key << "New Value:" << arg1;
...
void QVOptionsDialog::buttonBoxClicked(QAbstractButton *button)
{
    ...
    qDebug() << "=== Button Box Clicked === Role:" << role;
